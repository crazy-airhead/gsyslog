...
func (s *Rfc3164TestSuite) TestParser_Valid(c *C) {
	...
	c.Assert(obtained.Header, DeepEquals, expected)
}
...
func (s *Rfc3164TestSuite) TestParser_NoTimestamp(c *C) {
	...
	obtainedTime := obtained.Get("timestamp").(time.Time)
	s.assertTimeIsCloseToNow(c, obtainedTime)

	obtained.Set("timestamp", now) // XXX: Need to mock out time to test this fully
	expected := map[string]interface{}{
		"client":    "",
		"timestamp": now,
		"hostname":  "",
